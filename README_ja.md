...
- **tmux セッション内**で実行すること（`tmms post` は tmux のペインメタデータを読み取るため）
...
> **警告:** `-f <ファイル>` またはファイルパスを引数として指定した場合、メッセージが outbox に書き出された後、**元のファイルは削除されます**。これは設計上の仕様（安全な移動処理）です。ファイルを残したい場合は事前にバックアップしてください。
...
| `-f, --file <PATH>` | 送信する Markdown ファイルのパス。 |

> **警告:** ファイルからの入力（`-f` またはファイル引数）を使用した場合、メッセージが outbox に書き出された後、**元のファイルは完全に削除されます**。
